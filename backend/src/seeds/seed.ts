  ...
  try {
    ...
    await connection.query(
      `INSERT INTO Usuario (nombre1, nombre2, apellidoP, apellidoM, ci, correo, telefono, fechaNacimiento, contrasena, idRol, estado, estadoA, fechaA)
       VALUES ('Admin', NULL, 'General', NULL, '1234567', 'admin@cinelapaz.com', '76543210', '1985-03-15', ?, 'ADMINISTRADOR', 1, 1, CURDATE())
    ...
    if (adminId) {
      await connection.query('INSERT IGNORE INTO Administrador (idUsuario) VALUES (?)', [adminId]);
    }

    // Boletería
    await connection.query(
      `INSERT INTO Usuario (nombre1, nombre2, apellidoP, apellidoM, ci, correo, telefono, fechaNacimiento, contrasena, idRol, estado, estadoA, fechaA)
       VALUES ('María', 'Elena', 'Gonzales', 'Mamani', '7890123', 'boleteria@cinelapaz.com', '76543211', '1990-07-22', ?, 'BOLETERIA', 1, 1, CURDATE())
       ON DUPLICATE KEY UPDATE contrasena = VALUES(contrasena)`,
      [hashBol]
    );
    ...
    // Clientes demo
    const clientes = [
      { n1: 'Carlos', n2: null, aP: 'Mamani', aM: 'Condori', ci: '4567890', cor: 'cliente@cinelapaz.com', tel: '71234567', fn: '1995-01-10' },
      { n1: 'Ana', n2: 'Lucía', aP: 'Flores', aM: 'Ríos', ci: '5678901', cor: 'ana.flores@email.com', tel: '72345678', fn: '1998-06-15' },
      { n1: 'Luis', n2: null, aP: 'Quispe', aM: 'Torres', ci: '6789012', cor: 'luis.quispe@email.com', tel: '73456789', fn: '1992-11-20' },
      { n1: 'María', n2: 'Fernanda', aP: 'Vargas', aM: 'López', ci: '7890124', cor: 'maria.vargas@email.com', tel: '74567890', fn: '2000-03-08' },
      { n1: 'Pedro', n2: null, aP: 'Huanca', aM: 'Ramos', ci: '8901234', cor: 'pedro.huanca@email.com', tel: '75678901', fn: '1988-09-25' },
    ];

    for (const c of clientes) {
      await connection.query(
        `INSERT INTO Usuario (nombre1, nombre2, apellidoP, apellidoM, ci, correo, telefono, fechaNacimiento, contrasena, idRol, estado, estadoA, fechaA)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'CLIENTE', 1, 1, CURDATE())
         ON DUPLICATE KEY UPDATE contrasena = VALUES(contrasena)`,
        [c.n1, c.n2, c.aP, c.aM, c.ci, c.cor, c.tel, c.fn, hashCli]
      );
      const [cliRow] = await connection.query<any[]>('SELECT idUsuario FROM Usuario WHERE correo = ? LIMIT 1', [c.cor]);
      if (cliRow[0]) {
        await connection.query(
          'INSERT IGNORE INTO Cliente (idUsuario, nit, razonSocial) VALUES (?, ?, ?)',
          [cliRow[0].idUsuario, c.ci, `${c.n1} ${c.aP} ${c.aM}`]
        );
      }
    }
    ...
    for (const s of salas) {
      // Verificar si ya existen asientos para esta sala
    ...
    const peliculas = [
      ...
      { tit: 'Nosferatu', dir: 'Robert Eggers', sin: 'Una obsession gótica entre una joven y el antiguo vampiro Orlok.', dur: 132, clasi: '18', estreno: '2024-12-25', poster: 'https://image.tmdb.org/t/p/w500/5qGIxdEO841C0tPreYfUGK0SfgW.jpg' },
    ...
    // Mapear precios según tipo de sala
    const precios: Record<string, number> = { 'Estándar': 20, '3D': 30, 'VIP': 40 };

    // Generar funciones para hoy y los próximos 7 días
    const hoy = new Date();
    ...
    for (let dia = 0; dia < 7; dia++) {
      ...
      for (const sala of salasRows) {
        ...
        // 2-3 funciones por sala al día, espaciadas 15min mínimo
        ...
        for (let f = 0; f < funcsPorSala && horasDisponibles.length > 0; f++) {
          ...
          // Calcular hora fin (duración promedio 120 min)
    ...
    console.log(`  - ${2 + clientes.length} usuarios (1 admin, 1 boletería, ${clientes.length} clientes)`);
